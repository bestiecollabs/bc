...
function validate(jo){
  const errs=[]; if(!jo.name) errs.push("name required");
...
export async function onRequestPost({ request, env }) {
  ...
  await db.exec("BEGIN");
  try {
    const insBatch = await db.prepare(`INSERT INTO import_batches (source_uri,status) VALUES (?,'new')`).bind("inline:csv").run();
    const batchId = insBatch.meta.last_row_id;
    ...
    for (let r=1; r<rows.length; r++){
      ...
      for (const f of fields){ jo[f] = idx[f] != null ? (row[idx[f]] || "").trim() : ""; }
    ...
    const counts = await env.DB.prepare(`SELECT SUM(valid=1) as valid, SUM(valid=0) as invalid, COUNT(*) as total FROM import_rows WHERE batch_id=?`).bind(batchId).first();
