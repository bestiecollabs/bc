export function assertAdmin(env, request) {
  ...
  // Prefer Access or explicit header
  let email = request.headers.get("Cf-Access-Authenticated-User-Email")
           || request.headers.get("x-admin-email");

  // Fallback to login cookie
  if (!email) {
    const cookie = request.headers.get("Cookie") || "";
    const m = cookie.match(/(?:^|;\s*)bestie_email=([^;]+)/i);
    if (m) {
      try { email = decodeURIComponent(m[1]); } catch { email = m[1]; }
    }
  }

  if (!email) return null;
  return allow.includes(String(email).toLowerCase()) ? String(email) : null;
}
