...
export const onRequest: PagesFunction<{ DB: D1Database }> = async (ctx) => {
  ...
  // Accept either header (service token flows) or CF Access cookie (identity flows)
  ...
  if (method === "POST") {
    ...
    const ins = await ctx.env.DB.prepare("INSERT INTO users (email, role) VALUES (?1, 'user')").bind(email).run();
    const id = Number(ins.meta?.last_row_id ?? 0);
    const row = await ctx.env.DB.prepare("SELECT id, email, created_at FROM users WHERE id=?1").bind(id).first<Row>();
