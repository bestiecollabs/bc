export const onRequestGet = async ({ env, request }) => {
  ...
  // Ensure the brands table exists
  const exists = await env.DB.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='brands'").first();
  if(!exists){ return new Response(JSON.stringify({ ok:true, rows:[] }), { headers:{ "Content-Type":"application/json" }}) }

  // Read from brands only. Filter soft-deleted. Do not filter by status.
  const sql  = "SELECT id, name, slug, status, deleted_at FROM brands WHERE deleted_at IS NULL ORDER BY id DESC LIMIT ?";
