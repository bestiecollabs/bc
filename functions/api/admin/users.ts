...
type Row = { id: number; email: string; role: Role; created_at: number; deleted_at?: number|null };
...
export const onRequest: PagesFunction<{ DB: D1Database }> = async (ctx) => {
  ...
  // Routing inside the same file
  if (method === "GET") {
    // Active only by default
    const includeSuspended = searchParams.get("include_suspended") === "1";
    const sql = includeSuspended
      ? "SELECT id,email,role,created_at,deleted_at FROM users ORDER BY created_at DESC LIMIT 200"
      : "SELECT id,email,role,created_at,deleted_at FROM users WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT 200";
    const r = await db.prepare(sql).all<Row>();
    const items = (r.results ?? []).map(u => ({
      id: u.id, email: u.email, account_type: u.role, created_at: u.created_at, suspended: !!u.deleted_at
    }));
  ...
  if (method === "POST") {
    ...
    if (!action) {
      ...
      if (!["brand","creator","admin"].includes(role)) return json({ ok: false, error: "invalid_role" }, 400);
    ...
    if (action === "suspend") {
      ...
      const u = await db.prepare("SELECT id,email,role,deleted_at FROM users WHERE id=?1").bind(id).first<Row>();
      if (!u) return json({ ok: false, error: "not_found" }, 404);
      if (u.deleted_at) return json({ ok: true, suspended: 0 }); // already suspended
      ...
      await audit(db, actorEmail, "suspend", id, { email: u.email, role: u.role });
    ...
    if (action === "set_account_type") {
      const id = Number(body?.id ?? 0);
      const role = (String(body?.account_type ?? "").toLowerCase() as Role);
      if (!Number.isInteger(id) || id <= 0) return json({ ok: false, error: "invalid_id" }, 400);
      if (!["brand","creator","admin"].includes(role)) return json({ ok: false, error: "invalid_account_type" }, 400);

      // Guard: do not demote the last admin
      const before = await db.prepare("SELECT role FROM users WHERE id=?1").bind(id).first<Row>();
      ...
      await db.prepare("UPDATE users SET role=?1, updated_at=unixepoch() WHERE id=?2").bind(role, id).run();
      await audit(db, actorEmail, "set_account_type", id, { to: role });
  ...
  if (method === "DELETE") {
    ...
    await audit(db, actorEmail, "delete", id, { email: u.email, role: u.role });
...
async function activeAdminCount(db: D1Database, excludingId?: number): Promise<number> {
  if (excludingId) {
    const r = await db.prepare("SELECT COUNT(*) AS c FROM users WHERE role='admin' AND deleted_at IS NULL AND id<>?1").bind(excludingId).first<{c:number}>();
    return Number((r as any)?.c ?? 0);
  }
  const r = await db.prepare("SELECT COUNT(*) AS c FROM users WHERE role='admin' AND deleted_at IS NULL").first<{c:number}>();
  return Number((r as any)?.c ?? 0);
}
