...
export const onRequest: PagesFunction<{ DB: D1Database }> = async (ctx) => {
  ...
  if (method === "GET") {
    ...
    const items = (r.results??[]).map(u => ({
      id:u.id, email:u.email, account_type: u.is_admin ? "admin" : u.role,
  ...
  if (method === "POST") {
    ...
    if (!action) {
      ...
      // Upsert-like: try insert; if conflict, return existing
      try {
        const ins = await db.prepare("INSERT INTO users (email,role) VALUES (?1,?2)").bind(email,role).run();
        await audit(db, actorEmail, "create", Number(ins.meta?.last_row_id??0), { email, role });
    ...
    if (action === "suspend" || action === "unsuspend") {
      ...
      if (action === "suspend") {
        ...
        await audit(db, actorEmail, "suspend", id, {});
