(function(){
  ...
  async function runImport(commit){
    ...
    let batch;
    try{
      batch=await apiPost("/api/admin/import/brands/batches",{ mode: commit?"commit":"dry-run" });
    }catch(e){
      console.error(e); if(statusEl) statusEl.textContent="Auth error creating batch: "+e.message; return;
    }
    const sz=300;
    for(let i=0;i<rowsObjs.length;i+=sz){
      ...
      try{
        ...
        console.error(e); if(statusEl) statusEl.textContent="Rows upload failed: "+e.message; return;
      }
    ...
    try{
      ...
      console.error(e); if(statusEl) statusEl.textContent="Finalize failed: "+e.message;
    }
  }
  ...
  async function loadBrands(){
    ...
    try{
      // accept {items:[]} or {rows:[]} or array
      let data=null;
      for(const p of ["/api/admin/brands","/api/admin/brands/list"]){
        try{ data=await apiGet(p); if(data) break; }catch(e){ console.warn(e.message); }
      }
  ...
  document.addEventListener("DOMContentLoaded",function(){
    const dry=q("dryBtn")||document.querySelector('button#dryBtn, button:contains("Dry run")');
    const commit=q("commitBtn")||document.querySelector('button#commitBtn, button:contains("Commit")');
    const refresh=q("refreshBtn")||document.querySelector('button#refreshBtn, button:contains("Refresh")');
