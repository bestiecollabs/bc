export async function onRequestGet({ env, params, request }) {
  ...
  const rs = await env.DB.prepare(`SELECT id,row_num,parsed_json,errors_json,valid FROM import_rows WHERE batch_id=? ORDER BY row_num LIMIT ? OFFSET ?`).bind(batchId, limit, offset).all();
