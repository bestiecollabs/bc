/** POST /api/admin/brands/:id/delete -> soft delete by setting deleted_at */
export const onRequestPost: PagesFunction<{ DB: D1Database }> = async (ctx) => {
  ...
  try {
    const stmt = await env.DB.prepare(
      "UPDATE brands SET deleted_at = COALESCE(deleted_at, datetime('now')) WHERE id = ? AND deleted_at IS NULL"
    ).bind(id).run();
